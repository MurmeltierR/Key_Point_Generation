...
new_experiment = Experiment(workspace=ws, name='keypoint_twitter_pipeline')
...
input_ds = Dataset.get_by_name(ws, name='tweets_2percent', version=2)
...
myenv = Environment.get(ws, 'hyperdrive_test', version=9)
...
selected_tweets = PipelineData(name='selected_tweets', 
    datastore=datastore,
    output_mode='upload',
    output_path_on_compute='/selected_tweets/',
    pipeline_output_name='selected_tweets'
    )
...
# Parties: ['Conservative', 'Labour', 'Democratic Unionist Party', 'Liberal Democrat', 'Scottish National Party']

topic_query = 'climate change'
selected_party = str(['Conservative'])
...
topic_selection_step = PythonScriptStep(
    ...
    '--tweet_selection', selected_tweets,
    '--party_selection', selected_party],
    inputs=[input_ds.as_named_input(name='tweet_corpus')],
