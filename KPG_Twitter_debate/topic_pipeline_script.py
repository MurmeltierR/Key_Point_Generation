...
parser.add_argument('--selected_tweets', type=str)
...
path = os.path.join(args.selected_tweets, 'selected_tweets.csv')
...
def topic_modeling(df):
    ...
    text_list = df['text_processed_cluster'].tolist()
    ...
    return df, embeddings#, umap_embeddings

def intra_silhouette_cluster_distance(distance_matrix, labels, index):
    #Berechnung der durchschnittlichen Kosinusdistanz eines Datenpunktes zu allen anderen Datenpunkten seines Clusters
...
def inter_silhouette_cluster_distance(distance_matrix, labels, index):
    #Berechnung der durchschnittlichen Kosinusdistanz eines Datenpunktes zu allen Datenpunkten eines isoliert betrachteten anderen Cluster   
...
documents_per_topic = combined_df.groupby(['topic_assignment'])['text_processed_cluster'].apply(list)
